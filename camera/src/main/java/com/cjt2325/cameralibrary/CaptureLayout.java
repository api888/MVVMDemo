...
public class CaptureLayout extends FrameLayout {
    ...
    public CaptureLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {//ORIENTATION_PORTRAIT表示竖屏
            layout_width = outMetrics.widthPixels;//设置布局的宽度,宽度与屏幕等宽
        } else {
            layout_width = outMetrics.widthPixels / 2;
        }
        button_size = (int) (layout_width / 4.5f);//设置拍照按钮的大小
        layout_height = button_size + (button_size / 5) * 2 + 100;//设置该布局的高度
