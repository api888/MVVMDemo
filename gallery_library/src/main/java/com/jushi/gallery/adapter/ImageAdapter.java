package com.jushi.gallery.adapter;
...
//    private ArrayList<String> selects = new ArrayList<>();
    ...
    public void onBindViewHolder(final ImageVH vh, final int i) {
        ...
        if (show_select) {
            ...
            if (obj.isSelected()) {
                vh.iv.setBackgroundResource(R.drawable.checked_gallery);
            } else {
                vh.iv.setBackgroundResource(R.drawable.uncheck_gallery);
            }
        ...
        if (!list.get(position).isSelected()) { // 当前组件没有选中
            if (count == 1) {
                ...
                if (selects.size() < count) {
                    ...
                    Toast.makeText(context, context.getString(R.string.over_image_num), Toast.LENGTH_SHORT).show();
                }
            }
        } else { // 当前组件选中了
