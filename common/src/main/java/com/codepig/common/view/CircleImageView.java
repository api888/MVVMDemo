...
public class CircleImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public CircleImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
//        setLayerType(LAYER_TYPE_SOFTWARE, null);//取消硬件加速，某些机型图片背景会变黑
        mPaint = new Paint();
        mPaint.setAntiAlias(true);
        //取出attrs中我们为View设置的相关值
        TypedArray tArray = context.obtainStyledAttributes(attrs, R.styleable.GlideImageView);
        mBorderRadius = tArray.getDimensionPixelSize(R.styleable.GlideImageView_roundRadius, BODER_RADIUS_DEFAULT);
        type = tArray.getInt(R.styleable.GlideImageView_roundType, TYPE_CIRCLE);
        tArray.recycle();
    }
